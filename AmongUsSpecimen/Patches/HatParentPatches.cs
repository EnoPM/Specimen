...
internal static class HatParentPatches
{
    ...
    private static bool SetCustomHat(HatParent hatParent)
    {
        ...
        var dirPath = Path.Combine(CustomCosmeticsManager.HatsDirectory, "Test");
        if (!Directory.Exists(dirPath)) Directory.CreateDirectory(dirPath);
        var d = new DirectoryInfo(dirPath);
        var filePaths = d.GetFiles("*.png").Select(x => x.FullName).ToArray();
        var hats = CustomCosmeticsManager.CreateHatDetailsFromFileNames(filePaths, true);
        if (hats.Count <= 0) return false;
        try
        {
            hatParent.Hat = CustomCosmeticsManager.CreateHatBehaviour(hats[0], true);
        }
        catch (Exception err)
        {
            Specimen.Instance.Log.LogWarning($"Unable to create test hat \n{err}");
            return true;
        }
